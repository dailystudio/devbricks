...
public class DatabaseConnectivityDirectSQLImplTest extends ActivityTestCase {
	...
	private byte[] randomBytesArray(int length) {
		assertTrue(length > 0);
		
		byte[] bytes = new byte[length];
		for (int i = 0; i <length; i++) {
			bytes[i] = (byte)mRandom.nextInt(255);
		}
		
		return bytes;
	}
	
	private byte[] fillBytesArray(int length, byte sample) {
		assertTrue(length > 0);
	...
	public void testInsertDatabaseObject() {
		AbsDatabaseConnectivity connectivity = 
			new DatabaseConnectivityDirectSQLiteImpl(mTargetContext, SampleObject2.class);
		...
		final byte[] rbtyes = randomBytesArray(1024);
		
		object.setValue(SampleObject2.COLUMN_BIN, rbtyes);
		...
		Asserts.assertEquals(rbtyes, c.getBlob(c.getColumnIndex(SampleObject2.COLUMN_BIN.getName())));
	...
	public void testInsertDatabaseObjects() {
		AbsDatabaseConnectivity connectivity1 = 
			new DatabaseConnectivityDirectSQLiteImpl(mTargetContext, SampleObject1.class);
		...
		for (int i = 0; i < 5; i++) {
			if (i % 2 != 0) {
				...
				assertEquals(0.3 * i, c.getDouble(c.getColumnIndex("altitude")));
				Asserts.assertEquals(fillBytesArray(i, (byte)i), 
						c.getBlob(c.getColumnIndex("binary")));
				c.moveToNext();
			}
		}
		c.close();
	...
	public void testSimpleQuery() {
		...
		AbsDatabaseConnectivity connectivity = 
			new DatabaseConnectivityDirectSQLiteImpl(mTargetContext, QueryObject.class);
		assertNotNull(connectivity);
